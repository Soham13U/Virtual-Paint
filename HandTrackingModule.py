       ...
       # print(results.multi_hand_landmarks)
        ...
        if self.results.multi_hand_landmarks:
            for handLms in self.results.multi_hand_landmarks:
                if draw:
                     self.mpDraw.draw_landmarks(img, handLms, self.mpHands.HAND_CONNECTIONS)   #draw 21 red points on hands along with connections
        return img
        ...
        if self.results.multi_hand_landmarks:
            ...
            for id,lm in enumerate(myHand.landmark):
                       # print(id,lm)
                        ...
                        cx, cy =int(lm.x * w), int(lm.y * h) #calculate position of landmarks
